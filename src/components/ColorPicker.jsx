...
export default function ColorPicker({ currentColor, setCurrentColor, addToPalette, palette }) {
  ...
  const handleAddToPalette = () => {
    const colorExists = palette.find((p) => p.colorCode === currentColor);
    if (!colorExists) {
      addToPalette({colorCode:currentColor, label: nextLabel});
    } else {
  ...
  return (
    <>
      <Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
        ...
        ></MuiColorInput>
        <TextField label="Graph label" value={nextLabel} variant="outlined"       onChange={(event) => {
